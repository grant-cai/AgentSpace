...
def load_agent_profile(json_path: str = "personality_summary.json") -> str:
    """
    Load the tutor's personality profile from JSON and convert to prompt text.
    This combines structured data with natural language for the system prompt.
    """
    with open(json_path, 'r') as f:
        profile = json.load(f)
    
    grad = profile.get('education', {}).get('graduate')
    grad_line = (
        f"- Currently pursuing {grad['degree']} at {grad['university']}"
        if isinstance(grad, dict) and grad.get('degree') and grad.get('university')
        else ""
    )
...
{grad_line}
- Worked as math tutor at {profile['work_experience']['tutoring']['employer']} for {profile['work_experience']['tutoring']['duration']}
...
- Focus on: {', '.join(profile['tutoring_philosophy']['priorities'])} over rote memorization (unless pattern exists)
...
- **Problem-Solving Structure**: {' → '.join(profile['key_frameworks']['thesis_structure']['components'])}
  ({profile['key_frameworks']['thesis_structure']['rule']})
- **Solution Structure**: {profile['key_frameworks']['essay_structure']['model']} ({profile['key_frameworks']['essay_structure']['description']})
- **Reasoning**: {profile['key_frameworks']['evidence']['requirement']}
- **Work Steps**: {profile['key_frameworks']['body_paragraphs']['structure']}
...
def create_rag_chain(vectorstore):
    ...
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-pro",  # Stronger, higher cost
        temperature=0.7,
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", "Context from Jeremy's interview:\n{context}\n\nStudent question: {question}")
    ])
...
class GrantTutorAgent:
    ...
    def __init__(self, transcript_path: str, rebuild_db: bool = False):
        ...
        print("Jeremy Tutor Agent ready!")
    ...
    def interactive_session(self):
        ...
        print("\n" + "="*60)
        print("Jeremy's Math Tutoring Session")
        print("="*60)
        print("\nHi! I'm Jeremy. What would you like to work on today?")
        ...
        while True:
            ...
            if student_input.lower() in ['exit', 'quit', 'bye']:
                print("\nJeremy: Great session! Good luck with your essay!")
                break
            
            if student_input.lower() == 'clear':
                self.clear_history()
                print("\nJeremy: Okay, fresh start! What would you like to work on?")
                continue
